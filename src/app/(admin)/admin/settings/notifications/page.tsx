...
import { Landmark, Bell, Save, AlertCircle } from 'lucide-react';
...
export default function NotificationsSettingsPage() {
  ...
  const fetchTemplates = async () => {
    try {
      ...
      showToast(e.message || 'Error fetching notification templates', 'error');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSaveTemplate = async (e: React.FormEvent) => {
    ...
    try {
      await api.put(`/admin/notification-templates/${selectedTemplate._id}`, form);
      showToast(`Notification template "${selectedTemplate.name}" updated!`, 'success');
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="flex flex-col gap-1">
        <h2 className="text-xl font-extrabold text-slate-900 uppercase tracking-tight">System Notification Templates</h2>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        ...
        <div className="lg:col-span-4 bg-white p-6 rounded-2xl border border-slate-200 shadow-sm flex flex-col gap-4">
          ...
          <div className="flex flex-col gap-2 max-h-[500px] overflow-y-auto pr-1">
            {templates.length === 0 ? (
              <p className="text-slate-400 text-xs py-8 text-center">No notification templates seeded.</p>
            ) : (
              templates.map((template) => (
                ...
                >
                  <Bell size={16} className={selectedTemplate?._id === template._id ? 'text-primary' : 'text-slate-400'} />
        ...
        <div className="lg:col-span-8">
          {!selectedTemplate ? (
            <div className="bg-white border border-slate-200 rounded-2xl p-16 text-center flex flex-col items-center gap-3 shadow-sm">
              <Bell size={40} className="text-slate-300 animate-pulse" />
            ...
            <div className="bg-white p-6 sm:p-8 rounded-2xl border border-slate-200 shadow-sm flex flex-col gap-6">
              ...
              <div className="bg-slate-50 border border-slate-200 text-slate-655 p-4 rounded-xl text-xs flex gap-2.5 items-start">
                ...
                <div className="flex flex-col gap-1 leading-relaxed">
                  ...
                  <span>Use double curly braces to display activity variables, e.g. <code>{"{{amount}}"}</code> or other dynamic fields depending on the trigger.</span>
                </div>
              </div>
              ...
              <form onSubmit={handleSaveTemplate} className="flex flex-col gap-5">
                ...
                <div className="flex flex-col gap-1.5">
                  <label className="text-[10px] font-bold text-slate-500 uppercase">Alert Header Title</label>
                  <input
                    type="text"
                    required
                    value={form.title}
                ...
                <div className="flex flex-col gap-1.5">
                  <label className="text-[10px] font-bold text-slate-500 uppercase">Alert Content Message</label>
                  <textarea
                    required
                    rows={8}
                    value={form.content}
